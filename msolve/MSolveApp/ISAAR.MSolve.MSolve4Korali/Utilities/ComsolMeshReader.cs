...
namespace ISAAR.MSolve.MSolve4Korali.Utilities
{
                ...
                using (var sr = new StreamReader(filepath))
                {
                    ...
                    while (line != null)
                    {
                        ...
                        else if (status == ReadingStatus.FoundTet4Indices && !line.Contains("#") && !line.Equals(""))
                        {
                            ...
                            Debug.WriteLine("Status: Reading Tet4 Indices");
                        }
                        else if (status == ReadingStatus.FoundWedge6Indices && !line.Contains("#") && !line.Equals(""))
                        {
                            ...
                            Debug.WriteLine("Status: Reading Wedge6 Indices");
                        }
                        else if (status == ReadingStatus.FoundHexa8Indices && !line.Contains("#") && !line.Equals(""))
                        {
                            ...
                            Debug.WriteLine("Status: Reading Hexa8 Indices");
                        }
                        else if (line.Equals("") &&
                        ...
                        else if (status == ReadingStatus.ReadingTet4Indices)
                        {
                            ...
                            Debug.WriteLine("\tElement {0} - Domain: {1}", newTupple.Item1, newTupple.Item3);
                        ...
                        else if (status == ReadingStatus.ReadingHexa8)
                        {
                            ...
                            foreach (var node in nodes)
                                Debug.WriteLine("\tNode {0}{1}: ({2}, {3}, {4})", node.ID, " ", node.X.ToString("F2"), node.Y.ToString("F2"), node.Z.ToString("F2"));
                            //Increment id
                            id++;
                        }
